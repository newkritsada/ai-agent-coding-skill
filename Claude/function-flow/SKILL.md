...
5. **A comment naming a section is a function waiting to be extracted.** Instead of
   ...
   matters).
6. **Group functions in reading order** under section banners matching the flow:
   ...
   // ---------- report ----------
   ```
7. **Branching lives inside named functions, not the orchestrator.** A per-item
   ...
   ```
8. **Validate up front, fail fast.** Preconditions (config sanity, required rows,
...
9. **Tiny expression helpers are worth extracting too** when repeated:
   `formatLabel(item)` beats three copies of the same template literal.
10. **Error handling and cleanup live at the top level only** (transaction
