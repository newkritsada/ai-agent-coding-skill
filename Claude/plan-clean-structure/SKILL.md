...
Read the plan file (given path, else the last `Plan saved at:`). Decide scope from §3–§4 only, then read and follow exactly one skill:
...
Then execute §7 in order, updating each frontmatter todo `status` as you go. End with `Plan at: /abs/path`.
